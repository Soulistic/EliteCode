...
const Timer: React.FC<TimerProps> = () => {
    ...
    const startTimer = () => {
        setShowTimer(true);
        setTimeElapsed(0); // Reset timeElapsed to 0 before starting the timer
    ...
    return (
        <div>
            {showTimer ? (
                <div className='flex items-center space-x-2 bg-dark-fill-3 p-1.5 cursor-pointer rounded hover:bg-dark-fill-2' onClick={stopTimer}>
                    <div>{formatTime(timeElapsed)}</div>
                    <FiRefreshCcw />
                </div>
            ) : (
                <div className='flex items-center p-1 h-8 hover:bg-dark-fill-3 rounded cursor-pointer' onClick={startTimer}>
                    <svg
